...
export default function AdminSpacesPage() {
  ...
  const handleReactivate = async (spaceId) => {
    const token = localStorage.getItem("token");
    const inventoryId = localStorage.getItem("activeInventoryId");

    try {
      await axios.post(
        `${API}/spaces/admin/spaces/${spaceId}/reactivate`,
        { inventoryId },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {spaces.map((space) => (
                <tr
                  key={space.id}
                  className={`align-top ${
                    space.isActive ? "" : "bg-slate-50 text-slate-500"
                  }`}
                >
                  ...
                  <td className="px-6 py-4 text-sm">
                    <span
                      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${space.isActive ? "bg-emerald-100 text-emerald-700" : "bg-zinc-200 text-zinc-700"}`}
                    >
                      {space.isActive
                        ? space.isFinalized
                          ? "Ativo / Finalizado"
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      {space.isActive ? (
                        ...
                        <button
                          type="button"
                          onClick={() => handleReactivate(space.id)}
